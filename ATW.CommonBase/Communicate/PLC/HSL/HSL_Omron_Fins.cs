...
{
    public class HSL_Omron_Fins : ICommunicatePLC
    {
        ...
        /// 
        /// </summary>
        public OmronFinsNet PLC { get; set; }
        ...
        public HSL_Omron_Fins(string _IP, int _Port = 9600, string heartAddress = "E0.0")
        {
            Activate();
            IP = _IP;
            Port = _Port;
        ...
        private void start()
        {
            PLC = new OmronFinsNet(IP, Port);
            PLC.ConnectTimeOut = 1000;
            connect = (PLC as OmronFinsNet).ConnectServer();
        }
        ...
        {
            bool result = true;
            var write_Qty = Val.Length % 1900 > 0 ? (Val.Length / 1900) + 1 : (Val.Length / 1900);
            for (int i = 0; i < write_Qty; i++)
            {
                if (result)
                {
                    ...
                    else if (Val.Length % 1900 > 0 && i == write_Qty - 1)
                    {
                        byte[] buffer = new byte[Val.Length % 1900];
                        Array.Copy(Val, 1900 * i, buffer, 0, Val.Length % 1900);
                        result = PLC.Write(address.Split('.')[0] + "."
                            + Convert.ToString((i * 950 + Convert.ToUInt32(address.Split('.')[1]))),
                            buffer).IsSuccess;
                    }
                    else
                    {
                        byte[] buffer = new byte[1900];
                        Array.Copy(Val, 1900 * i, buffer, 0, 1900);
                        result = PLC.Write(address.Split('.')[0] + "."
                            + Convert.ToString((i * 950 + Convert.ToUInt32(address.Split('.')[1]))),
                            buffer).IsSuccess;
                    }
